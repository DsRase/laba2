...
8. Считаем итоговую потерю по пингу. Возвращаемся в окно из шага 5, ждём ~30 секунд после обрыва и прерываем пинг по `Ctrl+C` - `ping` сам распечатает строку статистики `N packets transmitted, M received, X% packet loss`. Если хочется команду без ручного прерывания, можно сделать отдельный замер с фиксированным числом пакетов:
   ```
   docker exec office_msk ping -c 30 172.16.99.2
   ```
   Ожидаем: общая потеря пакетов **меньше 10%** за весь интервал - это потери только в момент переключения, дальше связь стабильна. Сравнить с заданием 1, где после такого же `loss 100%` потери остаются 100% до конца.

9. Возвращаем оптику обратно:
   ```
